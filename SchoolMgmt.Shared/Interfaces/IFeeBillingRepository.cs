...
namespace SchoolMgmt.Shared.Interfaces
{
    ...
    int userId);
        ...
        Task<PaginatedResponse<dynamic>> GetInvoicesAsync(
            int orgId,
            int page,
            int size,
            string search); Task<(InvoiceHeaderDto header, IEnumerable<InvoiceItemDto> items, IEnumerable<PaymentAllocationDto> allocations)>
