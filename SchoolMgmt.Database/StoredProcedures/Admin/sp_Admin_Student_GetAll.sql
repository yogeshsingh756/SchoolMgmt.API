...
CREATE PROCEDURE `sp_Admin_Student_GetAll` (
...
BEGIN
    ...
    SET p_StudentType  = NULLIF(TRIM(IFNULL(p_StudentType, '')), '');
    ...
    IF p_IsDropdown = 1 THEN
        SELECT 
            ...
            u.Address,
            'Student' AS RoleName,
            CASE WHEN u.IsActive = 1 THEN 'Active' ELSE 'Inactive' END AS StudentStatus,
            u.CreatedOn,
            ...
            s.FatherName
        FROM Users u
            INNER JOIN Roles r ON u.RoleId = r.RoleId AND r.RoleName = 'Student'
            LEFT JOIN Users mu ON u.ModifiedBy = mu.UserId
            INNER JOIN Students s ON s.UserId = u.UserId AND s.OrganizationId = u.OrganizationId
        ...
        WHERE 
            ...
            AND (p_StudentType IS NULL OR s.StudentType = p_StudentType)
