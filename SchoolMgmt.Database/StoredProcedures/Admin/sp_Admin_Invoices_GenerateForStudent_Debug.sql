...
CREATE PROCEDURE `sp_Admin_Invoices_GenerateForStudent_Debug` (IN `p_OrganizationId` INT, IN `p_StudentId` INT, IN `p_ClassId` INT, IN `p_TermId` INT, IN `p_SessionId` INT, IN `p_InvoiceDate` DATE, IN `p_DueDate` DATE, IN `p_Notes` VARCHAR(255), IN `p_CreatedBy` INT)   BEGIN
