...
CREATE PROCEDURE `sp_Admin_ClassFeeMaster_GetAll` (IN `p_OrganizationId` INT, IN `p_PageNumber` INT, IN `p_PageSize` INT, IN `p_Search` VARCHAR(100))   BEGIN
  ...
  SELECT SQL_CALC_FOUND_ROWS
         cfm.ClassFeeId, cfm.ClassId, c.ClassName,
         cfm.Section, cfm.SectionId, sec.SectionName,
  ...
  LEFT JOIN Sections sec ON sec.SectionId = cfm.SectionId
  WHERE cfm.OrganizationId = p_OrganizationId
    ...
    AND (
         ...
         OR sec.SectionName LIKE CONCAT('%', p_Search, '%')
    )
  ORDER BY c.ClassName, sec.SectionName, t.TermName, ft.FeeTypeName
