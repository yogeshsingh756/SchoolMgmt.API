...
CREATE PROCEDURE `sp_Admin_Subjects_Update` (IN `p_OrganizationId` INT, IN `p_SubjectId` INT, IN `p_SubjectName` VARCHAR(150), IN `p_SubjectCode` VARCHAR(50), IN `p_Description` VARCHAR(500), IN `p_IsActive` TINYINT(1), IN `p_ModifiedBy` INT)   BEGIN
