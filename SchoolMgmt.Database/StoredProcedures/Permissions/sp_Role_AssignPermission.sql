...
CREATE PROCEDURE `sp_Role_AssignPermission` (IN `p_RoleId` INT, IN `p_PermissionId` INT, IN `p_CanView` TINYINT(1), IN `p_CanCreate` TINYINT(1), IN `p_CanEdit` TINYINT(1), IN `p_CanDelete` TINYINT(1), IN `p_ModifiedBy` INT)   BEGIN
