...
CREATE PROCEDURE `sp_AuditLog_Insert` (IN `p_OrganizationId` INT, IN `p_UserId` INT, IN `p_EntityName` VARCHAR(100), IN `p_ActionType` VARCHAR(50), IN `p_EntityKey` VARCHAR(100), IN `p_Description` TEXT, IN `p_OldValue` JSON, IN `p_NewValue` JSON, IN `p_IpAddress` VARCHAR(45), IN `p_UserAgent` VARCHAR(255))   BEGIN
