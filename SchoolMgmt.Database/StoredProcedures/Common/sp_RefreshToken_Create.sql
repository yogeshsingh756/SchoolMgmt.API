...
CREATE PROCEDURE `sp_RefreshToken_Create` (IN `p_OrganizationId` INT, IN `p_UserId` INT, IN `p_Token` VARCHAR(255), IN `p_ExpiresAt` DATETIME, IN `p_CreatedBy` INT, OUT `o_RefreshTokenId` BIGINT)   BEGIN
