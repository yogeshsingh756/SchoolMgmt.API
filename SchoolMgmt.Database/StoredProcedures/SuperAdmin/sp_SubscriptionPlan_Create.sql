...
CREATE PROCEDURE `sp_SubscriptionPlan_Create` (IN `p_PlanName` VARCHAR(150), IN `p_Description` TEXT, IN `p_Price` DECIMAL(10,2), IN `p_BillingCycle` VARCHAR(20), IN `p_CustomMonths` INT, IN `p_CreatedBy` INT, OUT `o_PlanId` INT)   BEGIN
